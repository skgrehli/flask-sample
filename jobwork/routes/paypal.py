...
jw_paypal=Blueprint('jw_paypal',__name__,url_prefix='')

@jw_paypal.route('/paypal_payment', methods=['POST'])
@authentication
def Pay():
    try :
        ...
        # 		credit_card = {"expire_year":card_details.credit_card_token.expire_year,"type":card_details.credit_card_token.type,"number":card_details.credit_card_token.number,"expire_month":card_details.credit_card_token.expire_month}
        # 	elif card_details.has_key('credit_card'):
        # 		credit_card = {"expire_year":card_details.credit_card.expire_year,"type":card_details.credit_card.type,"number":card_details.credit_card.number,"expire_month":card_details.credit_card.expire_month}
                ...
                if request.json['status'] == "selectedbyjobber" :
                    ...
                    if job_bid_detail is not None:
                        ...
                        if job_bid_detail['status'] == 'pending':
                            ...
                            #notificationtext="you reverse bid on this job title" +jobCollection['title']

                            notificationtext = "You reverse bid on the job title " +jobCollection['title'] + " for bidder : " + str(bidderfullname)
